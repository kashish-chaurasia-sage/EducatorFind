...
public function __construct(){
	...
	if($this->input->post('edu_per_submit')){
		...
		$this->load->view('become-educator');
		}
	...
	$edu_class=[];
	 ...
	 $edu_art[]=$artSql[0]->edu_lang_title;
        }
           }
        $lang_prof=[];
	...
	if(!empty($this->session->userdata('lang_coun'))){
		...
		$keyword=($edu_classArr ? $edu_classArr:'').','.($edu_boardArr ? $edu_boardArr:'').','.($edu_subArr ? $edu_subArr:'').','.($edu_examArr ? $edu_examArr:'').','.($edu_courseArr ? $edu_courseArr:'').','.($lang_academicArr ? $lang_academicArr:'').','.($edu_artArr ? $edu_artArr:'').','.((!empty($edu_art)) ? 'art':'').','.((!empty($edu_exam)) ? 'exam':'').','.(($academic == '1') ? 'academic':'').','.(($training =='1') ? 'professinoal':'').','.(($counselling =='1') ? 'career-counselling':'');
        ...
        $searchString=array(
		 ...
		 $LongJsonInfo = array(
							...
							'edu_exam'=>$exam?$exam:'',
							'career_path'=>$career_path?$career_path:'',
							'prof_course'=>$prof_course?$prof_course:'',
							'art'=>$art?$art:'',
